...
export class OtpService {
  ...
  async createPasswordResetOtp(userId: string): Promise<string> {
    ...
    try {
      ...
      this.logger.log(`Password reset OTP created for user ${userId}`);

      return savedOtp.otp_code;
    } catch (error) {
      await queryRunner.rollbackTransaction();
      this.logger.error(
        `Failed to create password reset OTP: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    } finally {
      await queryRunner.release();
  ...
  async verifyPasswordResetOtp(userId: string, otpCode: string): Promise<boolean> {
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    ...
    try {
      ...
      if (!otp) {
        this.logger.warn(`Invalid password reset OTP code for user ${userId}`);
        return false;
      }

      // Check if OTP is expired
      if (new Date() > otp.expires_at) {
        this.logger.warn(`Expired password reset OTP code for user ${userId}`);
        await queryRunner.manager.update(Otp, { id: otp.id }, { is_used: true });
        await queryRunner.commitTransaction();
        return false;
      }
      ...
      // Check if max attempts exceeded
      if (otp.attempts >= otp.max_attempts) {
        this.logger.warn(`Max attempts exceeded for password reset OTP ${otp.id}`);
        await queryRunner.manager.update(Otp, { id: otp.id }, { is_used: true });
        await queryRunner.commitTransaction();
        return false;
      }
      ...
      this.logger.log(`Password reset OTP verified successfully for user ${userId}`);
      return true;
    } catch (error) {
      await queryRunner.rollbackTransaction();
      this.logger.error(
        `Failed to verify password reset OTP: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    } finally {
      await queryRunner.release();
